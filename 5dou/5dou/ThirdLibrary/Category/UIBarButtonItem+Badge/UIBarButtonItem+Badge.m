...
NSString const *UIBarButtonItem_badgePadding_verticalKey = @"UIBarButtonItem_badgePadding_verticalKey";
NSString const *UIBarButtonItem_badgePadding_horizontalKey = @"UIBarButtonItem_badgePadding_horizontalKey";

@implementation UIBarButtonItem (Badge)

@dynamic badgeValue, badgeBGColor, badgeTextColor, badgeFont;
@dynamic badgeMinSize, badgeOriginX, badgeOriginY;
@dynamic horizontalBadgePadding, verticalBadgePadding;
...
//        defaultOriginX = superview.frame.size.width - self.badge.frame.size.width/2;
    ...
    self.badgeBGColor   = WDColorFrom16RGB(0xfa4141);
    self.badgeTextColor = WDColorFrom16RGB(0xfa4141);
    self.badgeFont      = [UIFont systemFontOfSize:9.0];
    self.horizontalBadgePadding   = 8;
    self.verticalBadgePadding = 1;
    self.badgeMinSize   = 8;
...
- (void)updateBadgeFrame
{
    ...
    //huangxin---添加内容
//    CGFloat verticalPadding = self.verticalBadgePadding;
//    CGFloat horizontalPadding = self.horizontalBadgePadding;
    // Using const we make sure the badge doesn't get too smal
    minWidth = (minWidth < minHeight) ? minHeight : expectedLabelSize.width;
    self.badge.layer.masksToBounds = YES;
    //huangxin----添加内容
//    self.badge.frame = CGRectMake(self.badgeOriginX, self.badgeOriginY, minWidth + horizontalPadding, minHeight + verticalPadding);
    self.badge.frame = CGRectMake(self.badgeOriginX, self.badgeOriginY, minHeight, minHeight);
...
-(UILabel*) badge {
    ...
    if(lbl==nil) {
        lbl = [[UILabel alloc] initWithFrame:CGRectMake(self.badgeOriginX, self.badgeOriginY, 8, 8)];
        [self setBadge:lbl];
        [self badgeInit];
...
// verticalBadgePadding value for the badge
-(CGFloat) verticalBadgePadding {
    NSNumber *number = objc_getAssociatedObject(self, &UIBarButtonItem_badgePadding_verticalKey);
    return number.floatValue;
}
-(void) setVerticalBadgePadding:(CGFloat)verticalBadgePadding
{
    NSNumber *number = [NSNumber numberWithDouble:verticalBadgePadding];
    objc_setAssociatedObject(self, &UIBarButtonItem_badgePadding_verticalKey, number, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (self.badge) {
        [self updateBadgeFrame];
    }
}
// horizontalBadgePadding value for the badge
-(CGFloat) horizontalBadgePadding {
    NSNumber *number = objc_getAssociatedObject(self, &UIBarButtonItem_badgePadding_horizontalKey);
    return number.floatValue;
}
-(void) setHorizontalBadgePadding:(CGFloat)horizontalBadgePadding
{
    NSNumber *number = [NSNumber numberWithDouble:horizontalBadgePadding];
    objc_setAssociatedObject(self, &UIBarButtonItem_badgePadding_horizontalKey, number, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (self.badge) {
        [self updateBadgeFrame];
    }
}
